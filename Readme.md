...
Container for JavaScript functions that test the state of inputs.
...
```
var inquire = require('atropa-inquire').inquire;
console.log(inquire);
```

In the browser this module is attached to the global namespace `atropa`, which
 will be created if it does not exist.
 Include `./browser/atropa-inquire_web.js` in your page and
 `atropa.inquire` will be available in your page.
...
To run the browser tests open `browser/atropa-inquire_tests.html` in your
...
If you alter the source in `src/atropa-inquire.js` please run the
